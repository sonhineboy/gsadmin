...
export default {
  ...
  methods: {
    ...
    async del(row, index) {
      ...
      var res = await this.$API.demo2.delete.post(reqData);
      if (res.code == 200) {
    ...
    batch_del() {
      ...
      this.$confirm("确定删除选中的"+this.selection.length+"项吗？", "提示", {
        ...
        .then(async () => {
          _that.loading = true;
          let res = await _that.$API.demo2.delete.post({ ids: ids });
          _that.loading = false;
    ...
    async confirm(data) {
      ...
      switch (this.$refs.myForm.mode) {
        case "add":
          re = await this.$API.demo2.save.post(data);
          break;
        case "edit":
          re = await this.$API.demo2.edit.post(data.id, data);
          break;
