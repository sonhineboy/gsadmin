...
export default {
  ...
  methods: {
    ...
    async del(row, index) {
      ...
      var res = await this.$API.test.delete.post(reqData);
      if (res.code == 200) {
    ...
    async confirm(data) {
      ...
      switch (this.$refs.myForm.mode) {
        case "add":
          re = await this.$API.test.save.post(data);
          break;
        case "edit":
          re = await this.$API.test.edit.post(data.id, data);
          break;
